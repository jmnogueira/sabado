...
void ordenarPersonas(sPersona listaPersonas[], int cant)
{
    int i;
    int j;
    char auxNombre[30];
    for(i=0; i<cant; i++){
        ...
        for(j=0; j<cant; j++){
           printf("%04d %20s \t %02d/%02d/%4d\n", listaPersonas[i].id)
           if(stricmp(listaPersonas[i].nombre,listaPersonas[j].nombre)){
                 strcpy(auxNombre[30], listaPersonas[i]);
                 listaPersonas[i], listaPersonas[j]);
                 listaPersonas[j])= auxNombre[30];
